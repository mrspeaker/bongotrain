...
;;; System allows up to 3 songs (just happens to be 3) to be defined at the same time.
;;; Each song is allowed to (try to) play on all three channels
;;; That's why sometimes song notes get cut off by sfx
    sng1_id           = $8042  ; 2 = dead, e = re/spawn, 6 = cutscene,
    sng2_id           = $8043  ; 7 = cutscene end dance, 9 = ?...
    sng3_id           = $8044  ;
    ...
    chA_tmp1         = $8052  ; All used in calc_mixed_note_vol_ch routines
    chB_tmp1         = $8053  ; Only used internally (no other refs)
    chC_tmp1         = $8054  ; TODO: not tmp, figure out what they are
    chA_tmp2         = $8055  ;
    chB_tmp2         = $8056  ;
    chC_tmp2         = $8057  ;
...
;;; Channel data structure:
    ...
    ;;; iy+1 = ?
    ;;; iy+2 = current volume
    ;;; iy+3 = "len" from sfx data meta
    ;;; iy+4 = new note flag
    ;;; iy+5 = envelope tick timer
    chA_data          = $82A0  ; base for sound 1 data
    chA_new_note_flag = $82A5  ; ix+4... only used directly in dbg func
    chB_data          = $82A8  ; base for sound 2 data
    chB_new_note_flag = $82AD  ; ix+4
    chC_data          = $82B0  ; base for sound 3 data
    chC_new_note_flag = $82B5  ; ix+4

    sng1_base         = $82B8  ; base of current struct for song1
    sng2_base         = $82D0  ; base of current struct for song2
...
;; Song data structure:
    ...
    ;; ix+$12 = note tick counter chA <- see, each song has counter for ALL channels
    ;; ix+$13 = note tick counter chB    (though, no other data? dunno)
...
reset_sfx_ids:
    push af
    push hl
    ld   hl,chA_cur_note        ; music note
    xor  a                      ; cp: If A == N, then Z flag is set
    cp   (hl)                   ; state == 0?
    jr   nz,_has_sfx_id         ; no, off to ADD
    inc  hl                     ; yep, what about $8067 (chB_cur_note)
    cp   (hl)                   ; == 0?
    jr   nz,_has_sfx_id
    inc  hl                     ; yep, what about $8068 (chC_cur_note)
    cp   (hl)                   ; == 0?
...
;;; I have not figured this out yet. I THINK the idea is that there
;;; can be up to 3 "songs" set at any one time (A song can be a
;;; sound effect, so more likely it's 1 song and 1 sound effect at a time).
;;; The AY-3-8910 has 3 channels, and each song can try and play
;;; on all three - if two songs try to play notes on the same channel,
;;; then someone gets cut off.

;;; Not sure this theory is correct though!

audio_system:
    ld   hl,write_aysnd_data_chA - JMP_HL_OFFSET
    call jmp_hl_plus_4k
    call reset_sfx_ids ; why reset after here, but not after next?
    ld   hl,write_aysnd_data_chB - JMP_HL_OFFSET
    call jmp_hl_plus_4k
...
;; Fades out each note over time
write_aysnd_chA_vol_fade:
    ld   a,(ix+$05)             ; env timer
    and  a
    jr   z,_408B
    dec  a
    ld   (ix+$05),a
...
_408B:
    ...
    add  a,$00
    ld   l,a
    ld   a,ay_chA_env_vol       ; write vol to aysnd
    out  (aysnd_write_0),a
    ld   a,l
    out  (aysnd_write_1),a
    nop
...
;;; Writes out the note course/fine pitch values
;;; as well as the volume envelope value
write_aysnd_chA_all:
    ld   a,(ix+$00)
    and  a
    ret  z
...
write_aysnd_chB_all:
    ...
    and  a
    ret  z
    ld   (chB_cur_note),a         ; current note
    sla  a
    ld   hl,sfx_note_lookup
    add  a,l
    ld   l,a
...
write_aysnd_data_chA:
    ...
    and  a
    jr   z,_i_2
    call write_aysnd_chA_all ; write new note and env
    ld   (ix+$04),$00
    ret
_i_2:              ; no new note, just update env
    call write_aysnd_chA_vol_fade
    ret
...
write_aysnd_data_chB:
    ...
    and  a
    jr   z,_i_3
    call write_aysnd_chB_all    ; write note
    ld   (ix+$04),$00 ;
    ret
_i_3:
    call write_aysnd_chB_vol_fade ; update vol
    ret
...
write_aysnd_data_chC:
    ...
    and  a
    jr   z,_i_4
    call write_aysnd_chC_all    ; write pitch
    ld   (ix+$04),$00
    ret
_i_4:
    call write_aysnd_chC_vol_fade ; update vol
    ret
...
;;; sfx channel C - write out note and envelope info
write_aysnd_chC_all:
    ld   a,(ix+$00)             ; has cur note?
    and  a
    ret  z                      ; nope, leave.
    ld   (chC_cur_note),a
    sla  a
    ld   hl,sfx_note_lookup     ; look up note course/fine vals
    add  a,l
    ld   l,a
    ld   a,ay_chC_tone
    out  (aysnd_write_0),a      ; write them to aysnd chip
    ld   a,(hl)
    out  (aysnd_write_1),a
    inc  hl
...
;;; ...Add transpose values and set note lengths based on velocity
;;; ...Also sets the new_note flag
;;; Called from:
;;;     chA_set_next_note
;;;     sfx_pattern_done
;;;     copy_cur_sfx_data_to_RAM
...
_433A:
    ...
    add  a,c        ;   a += init_a
    jr   _433A
_4340:              ; (now a = velocity * duration)
    dec  a          ; a -= 1
    ld   (ix+$12),a ; current chA note timer
    call calc_mixed_note_vol_chA
    ld   (iy+$02),a ; volume
    ld   a,(ix+$0e) ; "len" from meta
    ld   (iy+$03),a ; timer? copied to env timer later
    ld   a,$01
    ld   (iy+$04),a ; set new_note flag
    ret
...
_4380:              ; (now a = ix+$0f * b)
    dec  a
    ld   (ix+$13),a ; current chB note timer
    call calc_mixed_note_vol_chB
    ld   (iy+$02),a             ; volume
    ld   a,(ix+$0e)             ; "len" from meta
    ld   (iy+$03),a
    ld   a,$01
...
chA_tick_note:
    ld   a,(ix+$12)
    and  a
    jr   z,chA_set_next_note   ; first timer = 0
    dec  a
    ld   (ix+$12),a
    ret
...
chA_set_next_note:
    ...
    jr   z,sfx_pattern_done_chA
    ld   (ix+$07),l
    ld   (ix+$08),h
    call configure_chA
    ret

sfx_pattern_done_chA:
    ld   l,(ix+$01)
    ld   h,(ix+$02)
    inc  hl
    inc  hl
...
;;; _4552:
next_pattern_chA:
    cp   $FF                    ; any more patterns?
    ret  z                      ; no, leave.
;;; _4555:
    ld   (ix+$01),l             ; yep... next pattern
    ld   (ix+$02),h
    ld   (ix+$07),a
    inc  hl
    ld   a,(hl)
...
;;
next_pattern_chB:
    cp   $FF                    ; end of patterns?
    ret  z
    ld   (ix+$03),l             ; nope - more to come!
    ld   (ix+$04),h
    ld   (ix+$09),a
    inc  hl
    ld   a,(hl)
...
;;
sfx_pattern_done_chC:
    ld   l,(ix+$05)
    ld   h,(ix+$06)
    inc  hl
    inc  hl
    ld   a,(hl)
    cp   $EE                    ; pattern continuation
    jr   nz,next_pattern_chC
    inc  hl
    ld   a,(hl)
    ld   c,a
    ld   b,$00
...
;;; sng1_id, B, C are set to 0 if new_note flag can be 0, but the tune is still playing (I think
...
;;; maybe it's why sfx will cut off a tune.
    ...
    dec  a
    ret  z
    call configure_chB         ; Yep, configure sound 2
    ld   a,(ix+$0d)             ; +$0d ? 2?
    dec  a
    dec  a
    ret  z
    call configure_chC        ; Yep, configur sound 3
    ret
...
;;; see sng1_tick
;;; Gets here when there's no sng2_id
sng2_tick:
    ld   ix,sng2_base
    ld   a,(ix+$0d)
    and  a
...
;;; chX_tmpN vars not used outside of this func
...
calc_mixed_note_vol_chA:
    push ix                     ; ix is sng2_base
    pop  hl
    ld   a,l
    cp   $E8                    ; is sng2_base? (ie $82E8)
    jr   nz,_520F
    ld   a,$02                  ; yes, chC...
    ld   (chA_tmp2),a          ; what is 2 for?
    jr   _5225
_520F:                          ; ch A and B
    ld   a,(chA_tmp2)
    and  a
...
_5225:
    ld   a,l
    cp   $D0                    ; is sng2_base? (ie $82D0)
    jr   nz,_5231
    ld   a,$02                  ; yes chB
    ld   (chA_tmp1),a          ; what is 2 for?
    jr   _523D
_5231:
    ld   a,(chA_tmp1)          ; chA and C
    and  a
    jr   z,_523D
    dec  a
    ld   (chA_tmp1),a
...
;; See note on calc_mixed_note_vol_chA
calc_mixed_note_vol_chB:
    push ix
    pop  hl
    ld   a,l
    cp   $E8
